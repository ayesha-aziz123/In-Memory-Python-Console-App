# Quickstart Guide: Console Todo App

**Branch**: `001-console-todo-app` | **Date**: 2025-12-07 | **Spec**: specs/001-console-todo-app/spec.md
**Input**: Implementation Plan from `/specs/001-console-todo-app/plan.md`

## Overview

This guide provides quick instructions to set up and run the "Console Todo App" CLI application. This application is an in-memory Python 3.13+ command-line tool for managing todo tasks.

## Prerequisites

*   Python 3.13 or newer installed on your system.

## Setup Instructions

1.  **Clone the Repository**:
    ```bash
    git clone https://github.com/ayesha-aziz123/In-Memory-Python-Console-App.git
    cd In-Memory-Python-Console-App
    ```
    (Note: Replace with actual repository URL if different)

2.  **Navigate to the Application Directory**:
    The main application entry point is located in `src/todo/`.
    ```bash
    cd src/todo/
    ```

## Running the Application

Once in the `src/todo/` directory, you can run the application directly using Python.

### Available Commands

The application supports the following commands:

*   **`python main.py add <title> [description]`**: Adds a new todo task.
    *   Example: `python main.py add "Buy groceries" "Milk, Eggs, Bread"`
    *   Example: `python main.py add "Finish report"`
*   **`python main.py list`**: Lists all current todo tasks.
*   **`python main.py update <id> [--title <new_title>] [--description <new_description>]`**: Updates an existing task.
    *   Example: `python main.py update 1 --title "Buy organic groceries"`
    *   Example: `python main.py update 2 --description "Draft section 1 and 2"`
*   **`python main.py complete <id>`**: Marks a task as complete.
    *   Example: `python main.py complete 1`
*   **`python main.py incomplete <id>`**: Marks a task as incomplete.
    *   Example: `python main.py incomplete 1`
*   **`python main.py delete <id>`**: Deletes a task.
    *   Example: `python main.py delete 1`
*   **`python main.py help`**: Displays a help message with available commands.
*   **`python main.py exit`**: Exits the application.

## Example Usage

1.  **Add a task**:
    ```bash
    python main.py add "Learn Gemini CLI" "Explore new features and commands"
    ```
2.  **Add another task**:
    ```bash
    python main.py add "Prepare presentation"
    ```
3.  **List tasks**:
    ```bash
    python main.py list
    ```
    (You should see the two tasks listed with their IDs and statuses)

4.  **Mark the first task as complete**:
    ```bash
    python main.py complete 1
    ```
5.  **List tasks again to see the change**:
    ```bash
    python main.py list
    ```
6.  **Update the second task's title**:
    ```bash
    python main.py update 2 --title "Finalize presentation slides"
    ```
7.  **Delete the first task**:
    ```bash
    python main.py delete 1
    ```
8.  **List tasks one last time**:
    ```bash
    python main.py list
    ```
    (Only "Finalize presentation slides" should remain)

This quickstart guide provides a basic overview. Refer to the full documentation for more details.
